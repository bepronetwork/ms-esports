...
class CasinoContractETH{
    ...
    sendFundsToCasinoContract = async (ethAmount, options, callback) => {
        try{
            return await self.account.sendEther(
                ethAmount,
                self.contractAddress,
                null, 
                options,
            ...
            console.log(err)
            throw new Error(`Possibly the Owner Account Address : ${self.account.getAddress()} does not have ${ethAmount} ETH to send to the Contract (Providing Liquidity)`)
        }   
    }
